...
QFXShader :: QFXShader( const char *szvp, const char *szfp )
{
	GLint objectStatus;
	char nameBuffer[16];
...
void QFXShader :: SetParameter1i( int param, int value )
{
	if ( unsigned( param ) >= QFX_MAX_SHADER_UNIFORMS ) {
		QFXLog::Instance().Warning( "QFXShader::SetParameter4fv: parameter index out of range (%i)\n", param );
		return;
	...
	qglUniform1iARB( m_iUniforms[param], value );
}

void QFXShader :: PrintInfoLog( GLhandleARB object )
{
	int logMaxLen;
